...
class PyramidFeatures(nn.Module):
    ...
    def forward(self, inputs):
        ...
        P5_x = self.P5_1(C5)      # 用1 * 1的卷积降低ResNet每个layer输出的通道维度到256
        ...
        P4_x = self.P4_2(P4_x)       # 上采样后用3 * 3的卷积消除混叠效应

        P3_x = self.P3_1(C3)
        P3_x = P3_x + P4_upsampled_x
        P3_x = self.P3_2(P3_x)

        P6_x = self.P6(C5)       # P6直接通过对C5用步长为2的3*3卷积获得

        P7_x = self.P7_1(P6_x)
        P7_x = self.P7_2(P7_x)    # P7通过对P6用ReLU和步长为2的3*3卷积获得
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        features = self.fpn([x2, x3, x4])         # [P3, P4, P5, P6, P7], 分辨率依次降低一半, 通道数都为256

        regression = torch.cat([self.regressionModel(feature) for feature in features], dim=1)    # B * (5*H*W*A) * 4
        classification = torch.cat([self.classificationModel(feature) for feature in features], dim=1)   # B * (5*H*W*A) * K

        anchors = self.anchors(img_batch)    # 形状为[1, 5*K*A, 4], 这里的K代表每一层feature的空间位置的个数
